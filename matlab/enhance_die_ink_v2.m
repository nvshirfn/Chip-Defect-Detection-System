% Enhance DIE_INK images - recommended first version.
% Ink defects are usually dark spots/blobs, so this version keeps the
% enhancement mild: grayscale -> median denoise -> contrast adjustment.
% It saves a new YOLO dataset and copies labels unchanged.

clear; clc;

srcRoot = fullfile(pwd, "ChipDetection_single_class", "DIE_INK");
dstRoot = fullfile(pwd, "ChipDetection_single_class_enhanced_ink_v2", "DIE_INK");
...
for s = 1:numel(splits)
    ...
    fprintf("Enhancing %s DIE_INK images with V2: %d files\n", splitName, numel(imageFiles));
...
fprintf("\nDone.\n");
fprintf("Enhanced DIE_INK V2 dataset saved to:\n%s\n", dstRoot);
